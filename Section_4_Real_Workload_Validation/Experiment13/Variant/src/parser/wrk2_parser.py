#!/usr/bin/env python3
"""
wrk2 output parser for Experiment 12.
...
Usage:
  python3 wrk2_parser.py --runs-dir data/raw/ --output data/processed/csv/
"""
...
def parse_all_runs(runs_dir: Path, output_dir: Path):
    """Parse all run directories and write per-endpoint CSVs."""
    ...
    for run_dir in run_dirs:
        ...
        for endpoint in ENDPOINTS:
            ...
            row = {"run": run_name, "endpoint": endpoint, **metrics}
            all_rows[endpoint].append(row)
            if metrics["p99"] is not None:
                print(f"  {run_name}/{endpoint}: P99={metrics['p99']:.2f}ms  "
    ...
    fieldnames = ["run", "endpoint", "p50", "p75", "p90", "p95", "p99",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parse wrk2 output files for Experiment 12")
