...
# Experiment13 root = two levels above this script file (src/analysis/amplification.py)
_SCRIPT_DIR  = Path(__file__).resolve().parent          # src/analysis/
_EXP13_ROOT  = _SCRIPT_DIR.parent.parent                # Experiment13/
_EXP12_ROOT  = _EXP13_ROOT.parent / "Experiment12"      # ../Experiment12/
_DEFAULT_BASELINE = _EXP12_ROOT / "results" / "tables" / "summary.csv"
...
def main():
    ...
    parser.add_argument(
        ...
        help="Experiment 12 summary.csv (default: auto-resolved sibling Experiment12)",
    )
    parser.add_argument("--data",   required=True, help="data/processed/csv/ directory")
    parser.add_argument("--output", required=True, help="Output CSV path")
    args = parser.parse_args()

    # Resolve baseline: try as-given first (absolute or relative to CWD),
    # then fall back to the script-relative default.
    baseline_path = Path(args.baseline)
    if not baseline_path.is_absolute():
        baseline_path = Path.cwd() / baseline_path
    if not baseline_path.exists():
        baseline_path = _DEFAULT_BASELINE
        print(f"[WARN] --baseline path not found; trying auto-resolved: {baseline_path}")

    print(f"[INFO] Baseline: {baseline_path}")
    baseline = load_baseline(baseline_path)
    if not baseline:
        print(f"[ERROR] Could not load baseline from: {baseline_path}")
        return
